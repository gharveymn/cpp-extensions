...
int main(int, char**)
{
    ...
    assert(cpp14::mismatch(II(ia), II(ia + sa), II(ib), II(ib+sb))
            == (std::pair<II, II>(II(ia+3), II(ib+3))));

    assert(cpp14::mismatch(RAI(ia), RAI(ia + sa), RAI(ib), RAI(ib+sb))
            == (std::pair<RAI, RAI>(RAI(ia+3), RAI(ib+3))));


    assert(cpp14::mismatch(II(ia), II(ia + sa), II(ib), II(ib+2))
