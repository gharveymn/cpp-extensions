...
void test_ctor_throws()
{
  ...
  try {
    ...
    cpp17::uninitialized_move_n(values, N, It(p));
    assert(false);
  } catch (...) {}
...
void test_counted()
{
  ...
  auto ret = cpp17::uninitialized_move_n(It(values), 1, FIt(p));
  ...
  assert(p[0].value == 1);
  assert(values[0] == 0);
  ret = cpp17::uninitialized_move_n(It(values+1), N-1, FIt(p+1));
