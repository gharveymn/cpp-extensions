...
//  If they're all the same, we should get the value back.
    ...
    Tag y{0, "Zero-y"};
    Tag z{0, "Zero-z"};
    assert(eq(cpp17::clamp(x, y, z), x));
    assert(eq(cpp17::clamp(y, x, z), y));
  }
...
//  If the value is greater than 'hi', we should get hi back.
    ...
    Tag y{0, "Zero-y"};
    Tag z{1, "One-z"};
    assert(eq(cpp17::clamp(x, y, z), z));
    assert(eq(cpp17::clamp(y, z, x), z));
  }
  
  {
    ...
    constexpr T y = 0;
    constexpr T z = 1;
    static_assert(cpp17::clamp(x, y, z) == x, "" );
    static_assert(cpp17::clamp(y, x, z) == x, "" );
  }
