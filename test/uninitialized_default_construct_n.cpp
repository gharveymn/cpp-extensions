...
void test_ctor_throws()
{
  ...
  try {
    ...
    cpp17::uninitialized_default_construct_n(It(p), N);
    assert(false);
  } catch (...) {}
...
void test_counted()
{
  ...
  It e = cpp17::uninitialized_default_construct_n(It(p), 1);
  assert(e == It(p+1));
  assert(Counted::count == 1);
  assert(Counted::constructed == 1);
  e = cpp17::uninitialized_default_construct_n(It(p+1), 4);
  assert(e == It(p+N));
...
void test_value_initialized()
{
  ...
  int* p = pool;
  auto e = cpp17::uninitialized_default_construct_n(It(p), 1);
  assert(e == It(p+1));
  assert(pool[0] == -1);
  assert(pool[1] == -1);
  e = cpp17::uninitialized_default_construct_n(It(p+1), 4);
