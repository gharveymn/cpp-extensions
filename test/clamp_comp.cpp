...
{
  assert(&cpp17::clamp(v, lo, hi, c) == &x);
}
...
//  If they're all the same, we should get the value back.
    ...
    Tag y{0, "Zero-y"};
    Tag z{0, "Zero-z"};
    assert(eq(cpp17::clamp(x, y, z, comp), x));
    assert(eq(cpp17::clamp(y, x, z, comp), y));
  }
...
//  If it's the same as the upper bound, we get the value back.
    ...
    Tag z{1, "One-z"};
    assert(eq(cpp17::clamp(x, y, z, comp), x));
    assert(eq(cpp17::clamp(z, y, x, comp), z));
  }
...
//  If the value is between, we should get the value back
    ...
    Tag z{2, "Two-z"};
    assert(eq(cpp17::clamp(x, y, z, comp), x));
    assert(eq(cpp17::clamp(y, x, z, comp), x));
  }
...
//  If the value is less than the 'lo', we should get the lo back.
    ...
    Tag y{1, "One-y"};
    Tag z{2, "Two-z"};
    assert(eq(cpp17::clamp(x, y, z, comp), y));
    assert(eq(cpp17::clamp(y, x, z, comp), y));
  }
  {
//  If the value is greater than 'hi', we should get hi back.
    ...
    Tag y{0, "Zero-y"};
    Tag z{1, "One-z"};
    assert(eq(cpp17::clamp(x, y, z, comp), z));
    assert(eq(cpp17::clamp(y, z, x, comp), z));
  }
  
  {
    ...
    constexpr T y = 0;
    constexpr T z = 0;
//    static_assert(cpp17::clamp(x, y, z, std::greater<T>()) == y, "" );
//    static_assert(cpp17::clamp(y, x, z, std::greater<T>()) == y, "" );
  }
