...
void test_ctor_throws()
{
  ...
  try {
    ...
    cpp17::uninitialized_move(values, values + N, It(p));
    assert(false);
  } catch (...) {}
...
void test_counted()
{
  ...
  auto ret = cpp17::uninitialized_move(It(values), It(values + 1), FIt(p));
  assert(ret == FIt(p +1));
  assert(Counted::constructed == 1);
  assert(Counted::count == 1);
  assert(p[0].value == 1);
  assert(values[0] == 0);
  ret = cpp17::uninitialized_move(It(values+1), It(values+N), FIt(p+1));
  assert(ret == FIt(p + N));
